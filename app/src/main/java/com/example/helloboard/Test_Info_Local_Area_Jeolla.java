...
public class Test_Info_Local_Area_Jeolla extends AppCompatActivity {
    private static String TAG = "MyApplication_Test_Info_Local_Area_Jeolla";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_test_info_local_area_jeolla);

        mTextViewResult = (TextView) findViewById(R.id.textView_main_result);
        mTextView = (TextView) findViewById(R.id.textView_main_text);
        mTextView.setMovementMethod(new ScrollingMovementMethod());
        //스피너 정의
        final String[] local_category_list = {"전라도 9급 지방직(일행직) 경쟁률/합격선",
                "전라도 9급 지방직(사복직) 경쟁률/합격선",
                "전라도 9급 지방직(세무직) 경쟁률/합격선"};
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int position, long id) {
                if (position == 0) {
                    ...
                    GetData_Jeolla task = new GetData_Jeolla();
                    task.execute("http://ad66025.dothome.co.kr/getjson_Jeolla_general_ad.php");
                }
                if (position == 1) {
                    mTextView.setText(null);
                    GetData_Jeolla task = new GetData_Jeolla();
                    task.execute("http://ad66025.dothome.co.kr/getjson_Jeolla_social_welfare.php");
                }
                if (position == 2) {
                    mTextView.setText(null);
                    GetData_Jeolla task = new GetData_Jeolla();
                    task.execute("http://ad66025.dothome.co.kr/getjson_Jeolla_tax.php");
                }
    ...
    private class GetData_Jeolla extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = ProgressDialog.show(Test_Info_Local_Area_Jeolla.this,
